function  [cent, varargout]=FastPeakFind2(d, thres, filt ,edg, res, fid)
...
% image using NANCONV, so that with high probability there will be only one
...
if (nargin < 3)
    filt = (fspecial('gaussian', 12,1)); %if needed modify the filter according to the expected peaks sizes
end
...
if any(d(:))  ; %for the case of non zero raw image
    ...
    if any(d(:))   ; %for the case of the image is still non zero
        ...
        % smooth image
        d=nanconv(single(d),filt,'noedge','nanout') ;
