...
PUBLIC  double expLoopEnergy(int u1, int u2, int type, int type2,
			     short si1, short sj1, short sp1, short sq1, int i, int j, int p, int q);
PUBLIC  double expHairpinEnergy(int u, int type, short si1, short sj1,
				const char *string, int i, int j);
...
PRIVATE	double	**EX_P; /* experimental data bonuses: EX_P[i][j] is the bonus for pair i,j */
int part_exp_data_loaded = 0;
...
PUBLIC float pf_fold(char *sequence, char *structure)
{
  ...
  if (!part_exp_data_loaded) {
    EX_P = malloc(n*sizeof(double *));
    for(i=0;i<n;i++)
      EX_P[i] = malloc(n*sizeof(double));
    for(i=0;i<n;i++)
	    for(j=0;j<n;j++)
		EX_P[i][j] = 1;
  }
...
PUBLIC void load_pf_experimental_data(char *filename, int seq_len){
	double kT = (temperature+K0)*GASCONST;   /* kT in cal/mol  */
        int i, j = 0, k = 0, flag = 1;
	EX_P = malloc(seq_len*sizeof(double *));
	for(i=0;i<seq_len;i++)
		EX_P[i] = malloc(seq_len*sizeof(double));
        FILE *file = fopen( filename, "r");
        if(file == 0){
                printf("Could not open file for experimental data\n");
        }
        else{
                ...
                char c;
                for( i=0; data[i]; i++){
                        if( flag == 1 && k < seq_len && j < seq_len){
                                EX_P[k][j] = atof(&data[i]);
				EX_P[k][j] = exp(EX_P[k][j]*10/kT);
				flag = 0;
                        }
...
PRIVATE void pf_linear(char *sequence, char *structure)
{
  ...
  for (j=TURN+2;j<=n; j++) {
	...
	if (((type==3)||(type==4))&&no_closingGU) qbt1 = 0;
	  ...
	  qbt1 = expHairpinEnergy(u, type, S1[i+1], S1[j-1], sequence+i-1, i, j)*scale[u+2];/* add scale[u+2] */
	  ...
	  for (l=MAX(k+TURN+1,j-1-MAXLOOP+u1); l<j; l++) {
	    ...
	    if (type_2) {
	      ...
	      qbt1 += qb[iindx[k]-l] * (scale[u1+j-l+1] *
		      ...
		      S1[i+1], S1[j-1], S1[k-1], S1[l+1], i, j, k, l));
	    }
	  }
	}
...
PRIVATE void pf_circ(char *sequence, char *structure){
  ...
  for(p = 1; p < n; p++){
	...
	strncat(loopseq, sequence, p);
      ...
      qho += (((type==3)||(type==4))&&no_closingGU) ? 0. : qb[iindx[p]-q] * expHairpinEnergy(u, type, S1[q+1], S1[p-1],  loopseq, q, p) * scale[u];
	...
	for(l=lstart;l <= n; l++){
	    ...
	    qio += qb[iindx[p]-q] * qb[iindx[k]-l] * expLoopEnergy(ln2, ln1, rtype[type2], type, S1[l+1], S1[k-1], S1[p-1], S1[q+1], l, k, p, q) * scale[ln1+ln2];
	}
  ...
  if((S != NULL) && (S1 != NULL)){
	...
	for (j=i+TURN+1; j<=n; j++) {
	  ...
	  if (type&&(qb[ij]>0.)) {
	    ...
	    tmp2 = expHairpinEnergy(u, rt, S1[j+1], S1[i-1], loopseq, j, i) * scale[u];
		...
		ln2 = i - l - 1;
		if(ln1+ln2>MAXLOOP) continue;
		tmp2 += qb[iindx[k] - l]*expLoopEnergy(ln1, ln2, rt, rtype[type_2], S1[j+1], S1[i-1], S1[k-1], S1[l+1], j, i, k, l) * scale[ln1 + ln2];
	      }
	    }
		...
		ln2 = i - 1 + n - l;
		if(ln1+ln2>MAXLOOP) continue;
		tmp2 += qb[iindx[k] - l]*expLoopEnergy(ln2, ln1, rtype[type_2], rt, S1[l+1], S1[k-1], S1[i-1], S1[j+1], l, k, i, j) * scale[ln1 + ln2];
	      }
	    }
...
/* compute Boltzmann weight of a hairpin loop, multiply by scale[u+2] */
  ...
  //if(part_exp_data_loaded && i>0 && j>0) q *= EX_P[i-1][j-1]*EX_P[j-1][i-1];
  return q;
}
PUBLIC double expLoopEnergy(int u1, int u2, int type, int type2,
			     short si1, short sj1, short sp1, short sq1, int i, int j, int p, int q) {
/* compute Boltzmann weight of interior loop,
  ...
  if ((u1==0) && (u2==0)){ /* stack */
    if(part_exp_data_loaded  && i > 0 && j > 0 && p > 0 && q > 0){
      z = expstack[type][type2];
      z *= EX_P[i-1][j-1]*EX_P[p-1][q-1];
      //z *= EX_P[j-1][i-1]*EX_P[q-1][p-1];
    }
    else
      z = expstack[type][type2];
  }
...
char *pbacktrack_circ(char *seq){
  ...
  for(i=1; (i < n); i++){
	...
	strncat(loopseq, sequence, i);
      ...
      qt += qb[iindx[i]-j] * expHairpinEnergy(u, type, S1[j+1], S1[i-1],  loopseq, j, i) * scale[u];
	...
	for(l=lstart; (l <= n); l++){
	    ...
	    if(!type) continue;
	    type2 = rtype[type2];
	    qt += qb[iindx[i]-j] * qb[iindx[k]-l] * expLoopEnergy(ln2, ln1, type2, type, S1[l+1], S1[k-1], S1[i-1], S1[j+1], l, k, i, j) * scale[ln1 + ln2];
...
static void backtrack(int i, int j) {
  do {
    ...
    if (((type==3)||(type==4))&&no_closingGU) qbt1 = 0;
      ...
      qbt1 = expHairpinEnergy(u, type, S1[i+1], S1[j-1], sequence+i-1, i, j)*
  ...
  for (i=1; i<length; i++)
    for (j=i+TURN+3; j<=length; j++) {
      ...
      p *= expLoopEnergy(0,0,ptype[iindx[i]-j],rtype[ptype[iindx[i+1]-(j-1)]],
			 0,0,0,0,0,0,0,0)*scale[2];/* add *scale[u1+u2+2] */
      if (p>cutoff) {
	pl[num].i = i;
	pl[num].j = j;
	pl[num++].p = p;
