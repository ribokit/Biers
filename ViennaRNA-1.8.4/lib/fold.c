...
PRIVATE short  *S, *S1;
PRIVATE	double	**EX; /* experimental data bonuses: EX[i][j] is the bonus for pair i,j */
int exp_data_loaded = 0;
...
void load_experimental_data(char *filename, int seq_len){
        int i, j = 0, k = 0, flag = 1;
	EX = malloc(seq_len*sizeof(double *));
	for(i=0;i<seq_len;i++)
		EX[i] = malloc(seq_len*sizeof(double));
...
float fold(const char *string, char *structure) {
  int i, j, length, energy, bonus=0, bonus_cnt=0;
  ...
  if (!exp_data_loaded) {
    EX = malloc(length*sizeof(double *));
    for(i=0;i<length;i++)
      EX[i] = malloc(length*sizeof(double));
    for(i=0;i<length;i++)
	    for(j=0;j<length;j++)
		EX[i][j] = 0;
  }
