...
int main(int argc, char *argv[])
{
  ...
  if ((exp_data)&&(istty))
    printf("Input experimental data file name\n");
  ...
  do {				/* main loop: continue until end of file */
    ...
    if (exp_data) {
      exp_data_file = get_line(stdin);
      if (exp_data_file!=NULL){
        load_experimental_data(exp_data_file, length);
        load_pf_experimental_data(exp_data_file, length);
      }
      else
	fprintf(stderr, "missing experimental data filename\n");
    }
    if (fold_constrained) {
