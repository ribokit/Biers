function mohcaplot_biers_split( D, seqpos, ligpos, titl, ticksize, save_path, secstr, c, c2, secstr_f, sc_fc )
...
if ~exist( 'sc_fc','var') || isempty( sc_fc); sc_fc = 1; end;
...
for i = 1:pg
    ...
    for j = 1:4
        subplot(2,2,j);
        if (i-1)*4+j <= length(D);
            mohcaplot_biers_loop( D{(i-1)*4+j}/sc_fc, seqpos, ligpos, titl{(i-1)*4+j}, ticksize, secstr, c, c2, secstr_f );
        end;
    end;
    ...
    if exist( 'save_path', 'var' )
        if ~isempty( save_path )
            print( gcf, '-depsc2', '-loose', '-r300', [save_path,'_', num2str(i),'.eps']);
            fprintf( ['Created: ', save_path, '_', num2str(i),'.eps\n'] );
        end
    end
end;


function mohcaplot_biers_loop( D, seqpos, ligpos, titl, ticksize, secstr, c, c2, secstr_f )
...
%       c2        opt  = colorcode foreground ([1 0 1] 'm' default)
%       secstr_f  opt  = flag for secstr halfs (default 0 lower half, 1 upper half, 2 both)
%
% Clarence Cheng, 2014
%
...
xlabel( 'Reverse transcription stop position [5'']','fontsize',12,'fontweight','bold' );
ylabel( 'Cleaved and ligated position [3'']','fontsize',12,'fontweight','bold' );
hold on;
...
if exist( 'secstr', 'var' ) && ~isempty(secstr);
    ...
    if secstr_f == 0;
        area_pred = triu(area_pred);
    elseif secstr_f == 1;
        area_pred = tril(area_pred);
    end;
